...
# Function to build the standard northwestbound tunnel 
def build_tunnel_northwest(distance):
    for x in range(1,distance,1):
        send_command(f"/fill ~-2 ~-2 ~-1 ~3 ~3 ~-2 stone")
        send_command(f"/fill ~-1 ~ ~-1 ~2 ~1 ~-1 air")
        send_command(f"/tp ~-1 ~ ~-1")

        if (x % 5 == 0):
            send_command(f"/setblock ~ ~-1 ~ sea_lantern")
            send_command(f"/setblock ~4 ~-1 ~1 sea_lantern")
...
def main():
    ...
    if distance:
        ...
        build_tunnel_northwest(distance)
    else:
